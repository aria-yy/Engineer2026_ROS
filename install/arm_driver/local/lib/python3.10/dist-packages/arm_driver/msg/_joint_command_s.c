...
bool arm_driver__msg__joint_command__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("arm_driver.msg._joint_command.JointCommand", full_classname_dest, 42) == 0);
  }
  arm_driver__msg__JointCommand * ros_message = _ros_message;
  {  // joint_names
  ...
  {  // velocities
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->velocities.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * arm_driver__msg__joint_command__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("arm_driver.msg._joint_command");
  ...
  {  // velocities
    ...
    if (ros_message->velocities.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->velocities.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->velocities.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
